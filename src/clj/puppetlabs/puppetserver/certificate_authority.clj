...
(defn report-cert-event
  ...
  [report-activity-or-nil message subject certnames ip-address activity-type]
...
(schema/defn ^:always-validate
  ...
  (let [validity    (cert-validity-dates ca-ttl)
    ...
    (report-cert-event report-activity-or-nil msg signee certnames ip "signed")
  ...
  (let [[our-full-crl & rest-of-full-chain] (utils/pem->crls cacrl)
    ...
    (if (= 0 serial-count)
      ...
      (let [new-full-crl (utils/revoke-multiple our-full-crl
        ...
        (report-cert-event report-activity-or-nil msg signee certnames ip "revoked")))
...
(schema/defn ^:always-validate set-certificate-status!
  ...
  (if (= :signed desired-state)
    ...
    (revoke-existing-certs! settings [subject] report-activity-or-nil request)))
